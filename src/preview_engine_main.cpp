...
SystemParams params{};
MassMotionLayer mass_layer;
MotionActivityFilter motion_filter;
EventLayer event_layer;
TextureLayer texture_layer;
ResonanceLayer resonance_layer;
SpatialRenderer4 spatial_layer;
TiltPseudoForceModel tilt_layer;
MassState mass{};
...
void resetLayers() {
  ...
  motion_filter.configure(params);
  event_layer.configure(params);
  texture_layer.configure(params);
  resonance_layer.configure(params);
  spatial_layer.configure(params);
  tilt_layer.configure(params);
  tilt_layer.reset();
  mass = mass_layer.state();
  events = {};
  spatial = {};
  tilt = {};
}
...
EMSCRIPTEN_KEEPALIVE int preview_step(float dt_s, float ax, float ay, float az,
  ...
  const bool was_initialized = motion_filter.initialized();
  const auto input = motion_filter.process(sample, dt_s);
  if (input.action == MotionInputAction::RejectFrame) return 0;
  events = {};
  if (input.action == MotionInputAction::ResetNeutral) {
    // Same discontinuity policy as the production sensor boundary, no catch-up.
    resetLayers();
  } else if (input.action == MotionInputAction::Integrate) {
    const uint8_t steps = motionIntegrationSubstepCount(input.effective_dt_s, mass_layer.maxStableStepS());
    if (steps == 0) {
      resetLayers();
    } else {
      const float substep_s = input.effective_dt_s / steps;
      for (uint8_t step = 0; step < steps; ++step) {
        mass = mass_layer.updateWithActivity(sample, input.activity, substep_s);
        if (was_initialized) {
          const auto next = event_layer.update(mass, substep_s, events.items.size() - events.count);
          for (std::size_t i = 0; i < next.count && events.count < events.items.size(); ++i) {
            events.items[events.count++] = next.items[i];
          }
        }
      }
      tilt = tilt_layer.update(sample, mass, input.effective_dt_s);
    }
  }
  const auto textures = texture_layer.update(events, dt_s);
  const auto resonance = resonance_layer.update(textures);
  spatial = spatial_layer.update(resonance, dt_s);
  time_s += dt_s;
  ++frame_counter;
