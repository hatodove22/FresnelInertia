...
bool EspNowTelemetryProducer::sendSnapshot(
  ...
  esp_err_t result;
  if (snapshot.mass.heartbeat.enabled) {
    const auto packet = encodeEspNowTelemetryPacketV5(snapshot, sequence, resolved_);
    status_.packet_bytes = static_cast<uint16_t>(sizeof(packet));
    result = esp_now_send(kBroadcastAddress,
        reinterpret_cast<const uint8_t*>(&packet), sizeof(packet));
  } else if (snapshot.mass.granular_pile_active || snapshot.mass.pressure.enabled) {
