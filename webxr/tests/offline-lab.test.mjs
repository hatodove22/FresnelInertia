...
function fixture(factory) {
  ...
  const engine = new FakeEngine(), creates = [], states = [], orientations = [], applied = [], hookPresets = [];
  ...
  return { lab, engine, element, creates, states, orientations, applied, hookPresets, classes, meters, scene,
...
test("slow playback scales the input and model clocks together and pause freezes both", async () => {
  ...
  const checkStep = (wallDt, modelDt, expectedTime) => {
    ...
    near(f.scene.group.position.x, 0.004 * Math.sin(expectedTime * Math.PI * 10));
  ...
  const callCount = f.engine.calls.length;
...
test("water Shake uses a broad sway while other presets retain their shared 5 Hz input and body motion", async () => {
  ...
  for (const preset of presets) {
    for (const slow of [false, true]) {
      ...
      near(f.scene.group.position.x, 0.004 * Math.sin(time * omega));
