...
const ribbonSteps = 2;
// A plane intersects at most six box edges. Buffer capacity never depends on pose.
const maxVertices = 6 * edgeSegments * ribbonSteps * 6;
...
export class LiquidContactLine {
  ...
  private readonly lower: THREE.Vector3;
  private readonly upper: THREE.Vector3;
  ...
  constructor(size: THREE.Vector3) {
    ...
    this.lower = size.clone().multiplyScalar(-0.5);
    this.upper = size.clone().multiplyScalar(0.5);
  ...
  private writeRibbon(geometry: THREE.BufferGeometry, volume: ContainedVolume, width: number, height: number, foam: number) {
    ...
    const emit = (point: THREE.Vector3, across: number, variation: number) => {
      ...
      this.vertex.addScaledVector(volume.normal, Math.sin(across * Math.PI) * height).clamp(this.lower, this.upper);
