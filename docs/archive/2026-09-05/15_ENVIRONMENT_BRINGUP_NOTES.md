...
## 11. AtomS3 Live USB telemetry needed a larger HWCDC TX ring

On 2026-09-04, the AtomS3 production image repeatedly stranded a partial
canonical JSON frame only after entering Live. Replacing the USB cable did not
change the failure. A short isolation matrix showed:

- Live with audio OFF: no complete frame, 48 drops
- Idle with audio ON: 41 complete frames, no drops
- Live with audio ON: two complete frames, 44 drops

The ESP32-S3 HWCDC implementation used by the pinned Arduino core defaults to
a 256-byte TX ring, exactly matching the producer's maximum per-update chunk.
The production setup now requests a 4096-byte ring before `Serial.begin()`
when `HAPTICS_ENABLE_USB_TELEMETRY=1`. Runtime telemetry still boots OFF and
no physical output is armed by this allocation.

After that change, the same matrix produced 41, 41, and 40 complete frames
respectively, with zero drop, backpressure, console-interruption,
unterminated-partial, and serialization-error growth in every condition. A
subsequent static Gate 1 run captured 357 schema-valid frames over 36.349
seconds (348 Live and nine Safe Idle), with a 103 ms maximum gap and no event,
energy, actuator, audio-underrun, or USB-counter growth. The operator confirmed
that no vibration, abnormal sound, or abnormal heating was observed during the
powered static interval, and the completed evidence bundle passed the hardware
checker.
