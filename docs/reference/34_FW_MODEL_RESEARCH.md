...
調査日: 2026-09-06。対象: リモート側の統合前リビジョンの coherent AtomS3 FW、二つの接触面傾斜、四つの振動出力、共有状態を表示するWebクライアント。

2026-09-07の統合注記: 別PCで実装した静止／動摩擦付きの縮約砂pile、炭酸のcharge／burst／remainingとtiltキック、production-C++ Wasm Lab、表示専用の液面modeは、この調査時点のコードには含まれていない。以下の行番号・「現行」監査・追加候補はその時点のものとして読む。砂の保持や液面表示を一から追加する必要はなく、本稿の移動／静止粒交換χや物理液体modeは、既存の縮約pile／表示用modeとは別の拡張候補である。統合後の契約と優先順位は06・08・16に従い、受容済みデモの微調整を先にする。
...
この節は調査時点の設計候補であり、現在の作業順序ではない。
以下の「studio未実装」「一軸から2～4軸へ」は履歴として残す。
現在は振動・素材応答・傾き3係数を同時に評価する5軸探索と保存・再利用を
実装済みであり、分離探索へ戻さない。現在の契約は[06](../06_PARAMETER_MODEL.md#joint-preference-search)、
実装境界は[31](31_REUSABLE_VISUAL_ARCHITECTURE.md#preference-tuning-workspace)、
未実施の実機比較を含む結果は[16](../16_PROGRESS_STATUS.md)を参照する。
この調整手段自体を本機の研究目的や知覚的有効性の実証とはしない。
