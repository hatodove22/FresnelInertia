# 17 Parametric Container Haptics Model Spec v1.0

## 0. Purpose and repo alignment

This document defines a formal on-device haptic model for:
...
Initial target hardware remains:

- ESP32-S3 / M5StickS3
- MAX98360A x4 for the vibrotactile branch
- XL330-M077-T x2 for the low-frequency tilt branch baseline

This specification is the formal math companion to:

- `docs/03_PIPELINE_SPEC.md`
- `docs/06_PARAMETER_MODEL.md`
- `docs/14_TILT_PSEUDOFORCE_SPEC_REV2.md`
...
## 13. Implementation priorities and current repo status

Adopt this formal model in the same additive order already established by
`AGENTS.md` and `docs/08_IMPLEMENTATION_PLAN.md`.

1. Audio backend abstraction
   Status: baseline implemented in `AudioOutput4Ch`
2. Actuator sweep and resonance identification
   Status: baseline implemented in `RuntimeCalibrator`
3. Mass migration layer refinement
   Status: active refinement target for `MassMotionLayer`
...
7. Texture layer atoms
   Status: baseline implemented, still being tuned
8. Four-transducer spatial rendering
   Status: baseline implemented in `SpatialRenderer4`
9. Telemetry / recorder / replay
   Status: baseline implemented
10. XL330 tilt-plane integration
   Status: baseline implemented behind compile/runtime gates, still needs bench validation
11. Smartphone / HMD transports
   Status: SoftAP / HTTP / WebSocket baseline exists, HMD transport remains future work

## 14. Acceptance criteria

The formal model is considered successfully integrated when:
...
These criteria extend the validation intent in `docs/07_TEST_AND_VALIDATION.md`.
